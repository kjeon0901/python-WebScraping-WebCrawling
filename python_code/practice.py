...
df = df.rename(columns = colname).drop([1, 2]).reset_index().drop('index', axis=1)

# 조건에 해당하는 품목 찾기
''' 
# 동시에 만족
20000 < 2021년 수출금액 < 50000
2021년 수출증감률 >= 50%
2020년 수출증감률 < 10%
'''
findDf = df[(df['2021년 (5월)수출금액'].astype(float)>20000) & (df['2021년 (5월)수출금액'].astype(float)<50000) & (df['2021년 (5월)수출증감률'].astype(float)>=50.0) & (df['2020년수출증감률'].astype(float)<10.0)]
findDf.info() # object 타입이므로 ↑ 숫자 비교할 때 타입 캐스팅 해줘야 함. 
